$(function () {
	...
	$("#add-berita").on("click", function () {
		...
		$("#id").val("");
		$(".modal-title").html(
			'<i class="fas fa-newspaper"></i> Tambah Produk Laporan'
		);
				...
				aoColumnDefs: [
					{
						mRender: function (data, type, row) {
							...
							$rowData +=
                              ...
                              <div class="dropdown-menu" role="menu">
                                  <a class="dropdown-item" href="javascript:void(0)" onclick="editkat('` +
								row.id +
								`','` +
								row.nama +
								`')"><i class="far fa-edit"></i> Edit</a>
                                  <a class="dropdown-item" href="#" onclick="deletekat(` +
								row.id +
			...
			{ data: "kategori" },
			{
				...
				render: function (data, type, row, meta) {
					...
					for (var key in row.files) {
                      ...
                      <a href="` +
								file +
								`" class="link-secondary">Baca Online</a>
                      </p>
                      </div>`;
						} else if (key == 1) {
							$rowData +=

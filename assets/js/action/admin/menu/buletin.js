$(function () {
	...
	$("#save-buletin").on("click", function () {
		savedata(st);
	});
	...
	$.ajax({
		...
		success: function (result) {
			if (result.code == 1) {
					...
					aoColumnDefs: [
						{
							mRender: function (data, type, row) {
								...
								for (var key in row.files) {
									if (
										row.files[key].extension == "image/jpeg" ||
										row.files[key].extension == "image/png" ||
										row.files[key].extension == "image/jpg"
									) {
										$rowData +=
											`
						...
						},
						{
							mRender: function (data, type, row) {
								...
								$rowData += `<div class="card">
                                        <div class="card-body">`;
								for (var key in row.files) {
									if (row.files[key].extension == "application/pdf") {
										$rowData +=
						...
						},
						{
							mRender: function (data, type, row) {
								...
								var file = "";
								for (var key in row.files) {
									if (row.files[key].extension == "image/jpeg") {
										file = row.files[key].path + "/" + row.files[key].filename;
										idfile = row.files[key].id;
									} else if (row.files[key].extension == "application/pdf") {
								...
								if (stat == 1) {
									st =
										`<div class="dropdown-divider"></div><a class="dropdown-item" href="#" onclick="updatepublish(` +
										row.id +
										`,0)"><i class="fas fa-sign-out-alt"></i> No Publish</a>`;
								} else {
									st =
										`<div class="dropdown-divider"></div><a class="dropdown-item" href="#" onclick="updatepublish(` +
										row.id +
										`,1)"><i class="fas fa-sign-out-alt"></i> Publish</a>`;
								}
								// }
								...
								$rowData +=
									`
                                        ...
                                        <div class="dropdown-menu" role="menu">
                                          <a class="dropdown-item" href="#" onclick="editdong('` +
									row.id +
									`', '` +
									row.judul +
									`', '` +
				...
				var table = $("#listbuletin").DataTable();
...
function savedata(st) {
	...
	if (id) {
		...
		var baseurl = "updatedatabuletin";
		var msg = "Update Poster";
	} else {
		var baseurl = "savedatabuletin";
        ...
        <div class="card-body">
          <div class="form-group">
            ...
            <div class="text-center">
                <img id="blah_` +
		count +
		`" name="images_poster" class="img-fluid" src="assets/img/no-image.png" alt="picture">
                <canvas hidden id="myCanvas_` +
		count +
		`"/>
            </div>
            <div class="custom-file" style="margin-bottom: 10px;margin-top: 10px;">
              <input type="file" class="custom-file-input" id="image_` +
		count +
		`" name="image_input" onChange="pilihgambar(this)">
              <label class="custom-file-label" for="image_` +
		count +
		`">Pilih foto</label>
            </div>
          </div>

$(function () {
	...
	$("#iconpage > a").attr("class", "nav-link active");
	$("#iconpage > a > i").addClass("text-info");
	...
	$("#add-icon").on("click", function () {
		...
		$("#id").val("");
		$("#nama").val("");
		$(".modal-title").html('<i class="fas fa-icons"></i> Tambah Icon');
							...
							mRender: function (data, type, row) {
								...
								for (var key in row.files) {
                                  ...
                                  <div class="card" style="width:12rem;">
                                    <img id="" name="" class="img img-thumbnail" src="` +
										row.files[key].path +
										"/" +
...
function savedata(st) {
	...
	if (id) {
		...
		var baseurl = "updatedataicon";
		var msg = "Update Icon";
	} else {
		var baseurl = "savedataicon";
		var msg = "Tambah Icon";
	}
...
function deleteData(id, id_file, path) {
	...
	swalWithBootstrapButtons
		.fire({
			title: "Anda yakin, hapus Icon ini?",
			text: "",
			icon: "warning",
...
function editdong(id, nama, status, path, idfile) {
	$("#add-icon").trigger("click");
	$(".modal-title").html('<i class="fas fa-icons"></i> Edit Icon');
	$("#id").val(id);
	$("#idfile").val(idfile);
	$("#nama").val(nama);
	$("#stat").bootstrapSwitch("state", status == "1" ? true : false);
	...
	var elemen =
      ...
      <div class="card card-light card-outline">
        ...
        <div class="card-body">
          <div class="form-group">
            ...
            <div class="text-center">
                <img id="blah_` +
		count +
		`" name="images" class="img-fluid" src="assets/img/no-image.png" alt="picture">
		...
		`">Pilih foto</label>
          ...
          <input id="caption_` +
		count +
		`" name="caption" type="text" class="form-control" placeholder="Caption">
          </div>
        </div>
      </div>
    </div>`;
